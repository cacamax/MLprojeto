...
import DuplaSenaStatistics from "../statistics/DuplaSenaStatistics";
...
import { generateDuplaSenaCombinations } from "../CombinationsGenerator";

interface DuplaSenaPageProps {
  className?: string;
}

const DuplaSenaPage = ({ className = "" }: DuplaSenaPageProps) => {
  ...
  useEffect(() => {
    ...
    if (
      showAdvanced === "true" &&
      (!storedLotteryType || storedLotteryType === "duplasena")
    ) {
      // Gerar 50 combinações fixas do Gerador Avançado
      const advancedCombinations = generateDuplaSenaCombinations(50, true);
  ...
  const handleGenerateCombinations = (count: number, patterns?: any) => {
    const newCombinations = generateDuplaSenaCombinations(count, false, patterns);
    setCombinations(newCombinations);
    setShowCombinations(true);
  };

  const handleAdvancedGenerate = (count: number) => {
    const advancedCombinations = generateDuplaSenaCombinations(count, true);
    setCombinations(advancedCombinations);
    setShowCombinations(true);
  };
    ...
    <div className={`min-h-screen flex flex-col bg-background ${className}`}>
      ...
      >
        <Button
          onClick={() => navigate("/")}
          size="icon"
          className="h-12 w-12 rounded-full shadow-lg bg-[#FF6600] hover:bg-[#FF6600]/90 text-white"
