...
import MegaSenaStatistics from "../statistics/MegaSenaStatistics";
import CombinationsDisplay from "../CombinationsDisplay";
import Footer from "../Footer";
import { Button } from "../ui/button";
import AmazonAffiliateButton from "../AmazonAffiliateButton";

import { generateMegaSenaCombinations } from "../CombinationsGenerator";

interface MegaSenaPageProps {
  className?: string;
}

const MegaSenaPage = ({ className = "" }: MegaSenaPageProps) => {
  ...
  useEffect(() => {
    ...
    if (
      showAdvanced === "true" &&
      (!storedLotteryType || storedLotteryType === "megasena")
    ) {
      // Gerar 50 combinações fixas do Gerador Avançado
      const advancedCombinations = generateMegaSenaCombinations(50, true);
  ...
  const handleGenerateCombinations = (count: number, patterns?: any) => {
    const newCombinations = generateMegaSenaCombinations(count, false, patterns);
    setCombinations(newCombinations);
    setShowCombinations(true);
  };

  const handleAdvancedGenerate = (count: number) => {
    const advancedCombinations = generateMegaSenaCombinations(count, true);
    setCombinations(advancedCombinations);
    setShowCombinations(true);
  };
    ...
    <div className={`min-h-screen flex flex-col bg-background ${className}`}>
      ...
      >
        <Button
          onClick={() => navigate("/")}
          size="icon"
          className="h-12 w-12 rounded-full shadow-lg bg-[#003399] hover:bg-[#003399]/90 text-white"
